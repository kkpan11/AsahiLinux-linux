...
static void cd321x_typec_update_mode(struct tps6598x *tps, struct cd321x_status *st)
{
	...
	if (!(st->data_status & TPS_DATA_STATUS_DATA_CONNECTION)) {
		...
		eusb_data.eudo = le16_to_cpu(st->usb4_status.eudo);
...
static void cd321x_update_work(struct work_struct *work)
{
	...
	// TODO: do we need tps->lock here?

	mutex_lock(&cd321x->update_lock);
	struct cd321x_status st = cd321x->update_status;
	cd321x->update_status.status_changed = 0;
	cd321x->update_status.pwr_status_changed = 0;
	cd321x->update_status.data_status_changed = 0;
	mutex_unlock(&cd321x->update_lock);
	...
	if (usb_connection)
		new_role = (tps->data_status & TPS_DATA_STATUS_USB_DATA_ROLE) ? USB_ROLE_DEVICE : USB_ROLE_HOST;
	...
	if (!tps->partner) {
		...
		tps->partner = typec_register_partner(tps->port, &desc);
		if (IS_ERR(tps->partner)) {
			dev_err(tps->dev, "%s: failed to register partnet\n", __func__);
		}
...
static void cd321x_queue_status(struct tps6598x *tps)
{
	...
	cd321x->update_status.pwr_status_changed |= cd321x->update_status.pwr_status ^ tps->pwr_status;
	cd321x->update_status.data_status_changed |= cd321x->update_status.data_status ^ tps->data_status;
...
static irqreturn_t cd321x_interrupt(int irq, void *data)
{
	...
	// TODO: unclear why this is needed, maybe just leftover?
	/*if (event & APPLE_CD_REG_INT_DATA_STATUS_UPDATE)
